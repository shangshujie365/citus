...
static void SendCopyDataToPlacement(StringInfo dataBuffer, MultiConnection *connection,
									int64 shardId);
...
CopyFromWorkerNode(CopyStmt *copyStatement, char *completionTag)
{
	...
	masterConnection = GetNodeConnection(NEW_CONNECTION | CACHED_CONNECTION,
										 nodeName, nodePort);
	MarkConnectionBusy(masterConnection);
					 ...
					 bool stopOnFailure, bool useBinaryCopyFormat)
{
	...
	foreach(placementCell, finalizedPlacementList)
	{
		 ...
		 */
		connection = GetPlacementConnection(NEW_CONNECTION | CACHED_CONNECTION,
											placement);
...
SendCopyDataToAll(StringInfo dataBuffer, List *connectionList)
{
	...
	foreach(connectionCell, connectionList)
	{
		...
		MultiConnection *connection = transactionConnection->connection;
 ...
 */
static void
SendCopyDataToPlacement(StringInfo dataBuffer, MultiConnection *connection, int64 shardId)
{
...
EndRemoteCopy(List *connectionList, bool stopOnFailure)
{
	...
	foreach(connectionCell, connectionList)
	{
		...
		MarkConnectionIdle(transactionConnection->connection);
	}
}
